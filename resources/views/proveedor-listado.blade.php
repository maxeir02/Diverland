    ...
    <form method="POST" action="{{ url('proveedores/guardar') }}">
        ...
        <div class="row">
            ...
            <div class="col-12 mb-3">
                ...
                <textarea name="datos_facturacion" class="form-control">{{ $proveedor->datos_facturacion ?? '' }}</textarea>
            </div>
        </div>
        ...
        <tbody>
            @foreach($proveedores as $prov)
                <tr>
                    ...
                    <td>{{ $prov->correo }}</td>
                    <td>{{ $prov->contacto }}</td>
                    <td>{{ $prov->datos_facturacion }}</td>
                    <td>
                    <a href="{{ url('proveedores/editar/'.$prov->id) }}" class="btn btn-warning btn-sm">Editar</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
